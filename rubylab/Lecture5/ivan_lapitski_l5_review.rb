...
class Command
  ...
  begin
  ...
  rescue Errno::ENOENT # NOTE: Cool one!
    ...
    retry # NOTE: cool, but as you know what was writen into file by defalt it might be better to just reuse it, not parse the file again.
  end
  ...
  def self.my_error_method (debug) # NOTE: no spaces between method_name and brackets(
    debug ? NoMethodError : StandardError
  end

  def self.command_by_name # NOTE: all the logic in this GIANT method. It is bad because not testable or hard testable and complex to read.
    print ' > '
    begin
      ...
      File.open( @config_yaml[:output_log], 'a') {|f| f.write("#{Time # NOTE: maybe better to log errors into error.log not output
          .now.strftime('%H:%M')} - Execution stopped \n")}
      puts "\nGood Bye!"
      exit 0
    end
 ...
 > Command unknown, available commands:\n"
      ...
      File.open( @config_yaml[:output_log], 'a') {|f| f # NOTE: here and everywhere: multiline block - please use do..end form.
...
class UpTimeCommand < Command
  class << self
    ...
    def run
      ...
      time = IO.read( uptime_file ).split[0].to_i # NOTE: double read from file..
