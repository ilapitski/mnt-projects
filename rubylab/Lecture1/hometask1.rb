...
if hash0.values_at(:Green)[0] > hash0.values_at(:Yellow)[0]
  puts 'Green more then Yellow'
else
  puts 'Yellow more then Green'
end

if hash0.values_at(:Green)[0] > hash0.values_at(:Blue, :Red).inject(:+)
...
def need_blr(dollars_need0, rate0, fee0)
  (dollars_need0*rate0*(1 + fee0)).to_i
end
...
def ice_cream(rate2, i_have2, fee2, ice_cream_cost2)
  ice_cream_cost2 <= (((i_have2/(rate2*(1+fee2))).modulo(1))*(rate2*(1+fee2))).to_i
end
